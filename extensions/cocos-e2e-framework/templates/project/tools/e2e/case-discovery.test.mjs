...
test('discoverE2ECases loads distributed case metadata in deterministic id order', (t) => {
    ...
    writeAsset(repoRoot, 'e2e/sample-e2e.test.ts');
    writeCase(repoRoot, 'z.case.json', {
        id: 'z-case',
        title: 'Z case',
        automation: {
            scriptName: 'e2e/sample-e2e.test.ts',
            className: 'z_e2e',
        },
        tags: ['z'],
    });
    ...
    assert.equal(cases[0].scriptName, 'e2e/sample-e2e.test.ts');
...
test('createPreviewBundlePatterns includes scripts and automation class names once', () => {
    ...
    assert.deepEqual(patterns, ['e2e/sample-e2e.test.ts', 'one_e2e', 'two_e2e']);
});
...
function validCase(overrides = {}) {
    return {
        ...
        automation: {
            scriptName: overrides.scriptName ?? 'e2e/sample-e2e.test.ts',
