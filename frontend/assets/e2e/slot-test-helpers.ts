...
// @ts-ignore
import { expect, sleep, waitForNextFrame } from 'db://automation-framework/runtime/test-framework.mjs';
...
export const forcedBoard: Board = [
    ['A', 'A', 'A'],
    ['K', 'Q', 'K'],
    ['7', '7', '7'],
];

export async function prepareSlot(): Promise<void> {
    await loadMainScene();
...
export async function waitForLabel(name: string, expected: string, timeoutMs: number): Promise<void> {
...
export async function visualStep(): Promise<void> {
...
export function backendRequest<TPayload>(
    type: string,
    payload: TPayload,
    expectedType: string,
...
export function isOkPayload(payload: unknown): boolean {
